...
export function useMediaUpload(options: UseMediaUploadOptions = {}): UseMediaUploadResult {
  ...
  const tuning = useMemo<MediaUploadTuningOptions>(
    () => ({
      pipelineProgressShare: options.tuning?.pipelineProgressShare ?? DEFAULT_PIPELINE_PROGRESS_SHARE,
      maxParallelFileJobs: options.tuning?.maxParallelFileJobs ?? deviceDefaults.maxParallelFileJobs,
    ...
    try {
      ...
      if (!built.artifacts.length) {
        setJobs((prev) =>
          prev.map((j) =>
            j.id === jobId
              ? { ...j, status: 'error', progress: 0, error: 'Nothing to upload with the current settings.' }
              : j,
          ),
        )
        return
      ...
      const reportUploadProgress = () => {
        ...
        setJobs((prev) =>
          prev.map((j) =>
            j.id === jobId
              ? { ...j, status: 'uploading', progress: Math.max(0, Math.min(100, p)), error: undefined }
              : j,
          ),
        )
      }
      ...
      setJobs((prev) =>
        prev.map((j) =>
          j.id === jobId
            ? { ...j, status: 'uploading', progress: Math.round(tuning.pipelineProgressShare * 100), error: undefined }
            : j,
        ),
      )
      ...
      await Promise.all(
        built.artifacts.map(async (artifact, index) => {
          if (transportMode === 'custom') {
            ...
            if (!uploadHandler) throw new Error('`uploadHandler` is required when transport is "custom".')
            await uploadHandler({
              artifact,
              signal: ac.signal,
              onProgress: (pct) => {
      ...
      if (error instanceof DOMException && error.name === 'AbortError') {
        setJobs((prev) =>
          prev.map((j) => (j.id === jobId ? { ...j, status: 'idle', progress: 0, error: undefined } : j)),
        )
        return
      }
      const e = error instanceof Error ? error : new Error(String(error))
      setJobs((prev) =>
        prev.map((j) => (j.id === jobId ? { ...j, status: 'error', progress: 0, error: e.message } : j)),
      )
