import type { PipelineDefinition, PipelineInfoMessage, PipelineResult, PipelineSource } from '../core'
...
import { isHeicLike, isTiffLike, tryDecodeHeicToJpegFile, tryDecodeTiffToJpegFile } from './optionalDecoders'
...
function info(level: PipelineInfoMessage['level'], message: string, code?: string): PipelineInfoMessage {
...
export async function runDefaultBrowserPipeline(
  ...
  const log = (level: 'debug' | 'info' | 'warn' | 'error', message: string, extra?: unknown) => {
    ...
    const prefix = `[media-pipeline] ${input.name}`
    ...
    stages: [
...

      {
        ...
        run: async () => {
          ...
          if (!poster) {
            return {
              artifacts: [],
              info: [info('warn', `Could not generate a video poster for "${input.name}".`, 'poster_failed')],
              removeFromQueue: false,
            }
          }
          return {
...

      {
        ...
        run: () => {
          ...
          if (!opts.saveOriginal && (opts.saveOptimized || opts.saveThumbnails) && opts.fallbackToOriginal) {
            return {
              artifacts: [
                {
                  variant: 'original',
              ...
              info: [info('info', `SVG "${input.name}" uploaded as original (no client-side raster optimize).`, 'svg_original')],
              removeFromQueue: false,
            }
          }
        ...
        run: async () => {
          ...
          if (rawImage) {
            ...
            if (decoded) {
              ...
              return { artifacts: [], info: [info('warn', `RAW decode failed for "${input.name}".`, 'raw_decode_failed')], removeFromQueue: false }
            }
          } else if (heicLike) {
            ...
            if (decoded) {
              ...
              return {
                artifacts: [],
                info: [info('warn', `HEIC/HEIF decode unavailable for "${input.name}".`, 'heic_decode_missing_or_failed')],
                removeFromQueue: false,
              }
            }
          } else if (tiffLike) {
            ...
            if (decoded) {
              ...
              return {
                artifacts: [],
                info: [info('warn', `TIFF decode unavailable for "${input.name}".`, 'tiff_decode_missing_or_failed')],
                removeFromQueue: false,
              }
            }
          ...
          try {
            ...
            if (opts.fallbackToOriginal) {
              return {
                ...
                info: [info('warn', `Could not optimize "${input.name}" in this browser. Uploading original.`, 'optimize_failed')],
                removeFromQueue: false,
              }
            }
            return { artifacts: [], info: [info('warn', `Could not optimize "${input.name}" in this browser.`, 'optimize_failed')], removeFromQueue: false }
          }
        },
      },
