...
export async function tryDecodeHeicToJpegFile(source: File): Promise<File | null> {
  ...
  try {
    ...
    const out = await jpegFileFromImageData(img, `${source.name.replace(/\.(heic|heif)$/i, '')}.jpg`, {
      quality: 0.92,
    })
    if (out) return out
  } catch {
...
export async function tryDecodeTiffToJpegFile(source: File): Promise<File | null> {
  try {
    ...
    const data = new Uint8ClampedArray(rgba.buffer.slice(rgba.byteOffset, rgba.byteOffset + rgba.byteLength))
    const out = await jpegFileFromImageData({ data, width, height }, `${source.name.replace(/\.(tif|tiff)$/i, '')}.jpg`, {
      quality: 0.92,
    })
    return out
  } catch {
