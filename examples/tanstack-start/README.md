...
TanStack Start app with **resumable uploads** over **TUS** (`/api/tus/`) and a **client-only** media pipeline from the **`media-pipeline`** workspace package (no server-side transcoding).
...
2. Queue builds in memory; **Start upload** runs `media-pipeline`’s default browser pipeline per file, then uploads each artifact with `tus-js-client`.
...
| Client TUS endpoint | `media-pipeline` hook defaults to `origin + /api/tus/`; override via `tus.endpoint` on `MediaUploadField` |
...
| `src/components/media-upload/MediaUploadField.tsx` | UI + `useMediaUpload` from `media-pipeline/react` |
